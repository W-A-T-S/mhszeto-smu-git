...
def view_enrolled_courses(learner_username):
    ...
    many_learner_assign_or_enroll_object = learnerAssignOrEnrolDAO.find_query(query={"_id.learner_username": learner_username, "is_enrolment_approved":True})
    ...
    for one_learner_assign_or_enroll_object in many_learner_assign_or_enroll_object:
        ...
        enrolled_courses_dict = {"course_id": course_id, "class_id":class_id, "course_description": course_description,"start_datetime":start_datetime, "end_datetime":end_datetime,"lesson_count":lesson_count,"learner_username":learner_username  }
        enrolled_courses_list.append(enrolled_courses_dict)
        
    # return render_template("view_enrolled_course.html", enrolled_courses_list)
    return render_template("learnerViewEnrolledCourses.html", courses=enrolled_courses_list)

@app.route("/view_completed_courses/<string:learner_username>")
def view_completed_courses(learner_username):
    learnerAssignOrEnrolDAO = LearnerAssignOrEnrolDAO()
    many_learner_assign_or_enroll_object = learnerAssignOrEnrolDAO.find_query(query={"_id.learner_username": learner_username, "is_completed":True})
    ...
    for one_learner_assign_or_enroll_object in many_learner_assign_or_enroll_object:
        ...
        end_datetime = one_class_obj.get_end_date_time()
        course_description = courseDAO.find_one(course_id).get_description()
        completed_courses_dict = {"course_id": course_id, "class_id":class_id, "course_description": course_description,"start_datetime":start_datetime, "end_datetime":end_datetime,"learner_username":learner_username  }
        completed_courses_list.append(completed_courses_dict)
        
    return render_template("learnerViewCompletedCourses.html", courses=completed_courses_list)
...
def get_learner_progress(class_id, course_id, learner_username):
    ...
    lessonDAO = LessonDAO()
    tl_lesson_count=len(lessonDAO.find_query(query={"_id.class_id": class_id, "_id.course_id": course_id}))
    ...
    if lesson_count != 0 and tl_lesson_count!=0:
        if lesson_count == tl_lesson_count:
            learnerAssignOrEnrolDAO.update_one(queryIdentifier={"_id.learner_username":learner_username, "_id.course_id":course_id, "_id.class_id": class_id}, queryValues={ "$set": {"is_completed":True}})
        else:
            learnerAssignOrEnrolDAO.update_one(queryIdentifier={"_id.learner_username":learner_username, "_id.course_id":course_id, "_id.class_id": class_id}, queryValues={ "$set": {"is_completed":False}})
